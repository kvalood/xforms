<section class="mini-layout">
    <div class="frame_title clearfix">
        ...
        <div class="pull-right">
            <div class="d-i_b">
                <a href="/admin/components/cp/xforms/" class="t-d_n m-r_15"><span class="f-s_14">←</span> <span class="t-d_u">{lang("Back","admin")}</span></a>
                <button onclick="$('#delete_fields').modal();" type="button" class="btn btn-small btn-danger action_on pages_action pages_delete" disabled="disabled"><i class="icon-trash"></i>Удалить</button>
                <a href="/admin/components/cp/xforms/field/{$form_id}" class="btn btn-small pjax btn-success"><i class="icon-plus-sign icon-white"></i>{lang("Create","admin")}</a>
            </div>
        </div>
    </div> 
    {if $fields}
            ...
            <tbody class="sortable save_positions" data-url="/admin/components/cp/xforms/update_positions">
                {foreach $fields as $field}
                    <tr>
                        ...
                        <td>
                            <div class="frame_prod-on_off" data-rel="tooltip" data-placement="top" data-original-title="{if $field.visible}{lang("show","admin")}{else:}{lang("don't show", 'admin')}{/if}" onclick="xforms.change_field_visible('{$field.id}');">
                                <span class="prod-on_off {if !$field.visible}disable_tovar{/if}" style="{if !$field.visible}left: -28px;{/if}"></span>
                            </div>
                        </td>
...
<div class="modal hide fade products_delete_dialog" id="delete_fields">
    ...
    <div class="modal-footer">
        ...
        <a href="" class="btn btn-primary" onclick="xforms.deleteFieldsConfirm();$('.modal').modal('hide');">{lang("Delete","admin")}</a>
    </div>
</div>
